...
def get_dataset(dataset):
    ...
    if dataset == 1:
        ...
        train_label_file = r'./datasets/2013houston/TrainImage.mat'
        data_HSI_mat = sio.loadmat(image_file_HSI)
        data_HSI = data_HSI_mat['HSI_data']
        data_LiDAR_mat = sio.loadmat(image_file_LiDAR)
        data_LiDAR = data_LiDAR_mat['LiDAR_data']

        gt_mat = sio.loadmat(label_file)
        gt = gt_mat['All_Label']
        train_gt_mat = sio.loadmat(train_label_file)
        train_gt = train_gt_mat['TrainImage']
    ...
    if dataset == 2:
        ...
        train_label_file = r'./datasets/Trento/TrainImage.mat'
        data_HSI_mat = sio.loadmat(image_file_HSI)
        data_HSI = data_HSI_mat['HSI_data']
        data_LiDAR_mat = sio.loadmat(image_file_LiDAR)
        data_LiDAR = data_LiDAR_mat['LiDAR_data']
    ...
    if dataset == 3:
        ...
        train_label_file = r'./datasets/Muufl/All_Label.mat'
        data_HSI_mat = sio.loadmat(image_file_HSI)
        data_HSI = data_HSI_mat['HSI_data']
        data_LiDAR_mat = sio.loadmat(image_file_LiDAR)
        data_LiDAR = data_LiDAR_mat['LiDAR_data']
        ...
        gt = gt_mat['All_Label']
        train_gt_mat = sio.loadmat(train_label_file)
        train_gt = train_gt_mat['All_Label']
        ...
        learning_rate = 2e-4  # 学习率
        max_epoch = 1000 # 迭代次数
        dataset_name = "Muffl"  # 数据集名称
        trainloss_result = np.zeros([max_epoch + 1, 1])
        LiDAR_bands = 1
        pass
    if dataset == 4:
        data_HSI_mat = sio.loadmat('data/Augsburg_SAR/HSI_data.mat')
        data_HSI = data_HSI_mat['HSI_data']
        data_LiDAR_mat = sio.loadmat('data/Augsburg_SAR/SAR_data.mat')
        data_LiDAR = data_LiDAR_mat['SAR_data']
        gt_mat = sio.loadmat('data/Augsburg_SAR/All_Label.mat')
        gt = gt_mat['All_Label']
        train_gt_mat = sio.loadmat('data/Augsburg_SAR/TrainImage.mat')
        train_gt = train_gt_mat['TrainImage']
...
def gen_model_data(data_HSI, data_LiDAR, patchsize_HSI, patchsize_LiDAR, train_label, test_label, D_label, batchsize):
    ...
    [ind1, ind2] = np.where(D_label != 0)  # ind1和ind2是符合where中条件的点的横纵坐标
    DNum = len(ind1)
    D_Patch_LiDAR = np.empty((DNum, 1, patchsize_HSI, patchsize_HSI), dtype='float32')
...
def data_partition(samples_type, class_count, gt, train_gt, train_ratio, val_ratio, height, width):
    ...
    if samples_type == 'same_num':  # 取固定数量训练
        ...
        test_data_index = list(test_data_index)
        train_data_index = list(train_data_index)
        # sio.savemat("train_indices.mat", {'train_indices': np.array(train_data_index)})
        # sio.savemat("test_indices.mat", {'test_indices': np.array(test_data_index)})
